...
class SessionContext:
    ...
    range_finalized_at: Optional[datetime] = None
...
class SessionManager:
    ...
    def _candle_in_range_window(self, candle: Candle, config: SessionConfig) -> bool:
        """Check if a candle's own NY-local start time is inside the range window.

        Range membership must be decided by the candle's timestamp, not by
        wall-clock time — otherwise the pre-range candle that completes exactly
        at range_start (e.g. 17:55-18:00 for Tokyo) leaks into the opening range.
        ...
        Returns:
            True if range_start <= candle_start (NY) < range_end.
        """
        t = candle.time.astimezone(NY_TZ).time()
        return config.range_start <= t < config.range_end
        ...
        if ctx.state == SessionState.WAITING_FOR_SESSION:
            ...
            if not self._is_in_range_period(current_time, config):
                ...
                if ctx.opening_range:
                    range_size_pct = (ctx.opening_range.high - ctx.opening_range.low) / ctx.opening_range.low * 100
                    if range_size_pct < TradingConfig.MIN_RANGE_SIZE_PCT:
                        logger.info(
                            f"[{ctx.session.value}] {self.instrument} -> SESSION_CLOSED "
                            f"(range too small: {range_size_pct:.3f}% < {TradingConfig.MIN_RANGE_SIZE_PCT}%)"
                        )
                        ctx.state = SessionState.SESSION_CLOSED
                        return None
                    ctx.range_finalized_at = self.get_current_time_ny()
            ...
            # Check TTB timeout
            if ctx.range_finalized_at:
                ttb_mins = (self.get_current_time_ny() - ctx.range_finalized_at).total_seconds() / 60
                if ttb_mins >= TradingConfig.MAX_TIME_TO_BREAK_MINS:
                    ctx.state = SessionState.SESSION_CLOSED
                    logger.info(
                        f"[{ctx.session.value}] {self.instrument} -> SESSION_CLOSED "
                        f"(no breakout within {TradingConfig.MAX_TIME_TO_BREAK_MINS}m, TTB={ttb_mins:.0f}m)"
                    )
                    return None
    ...
    def _close_trade(
        ...
        # Reset breakout direction to allow re-entry ONLY after a stop-out
        # (matches backtest.py: target/time exits end the session, no re-entry).
        if reason == "stop" and ctx.breakout_detector:
    ...
    def handle_trade_exit(
        ...
        if ctx.state == SessionState.IN_TRADE and ctx.current_trade:
            ...
            # Re-entry is allowed ONLY after a stop-out and while under the
            # per-session trade cap (matches backtest.py). Target/time exits
            # end the session for this instrument.
            if reason == "stop" and ctx.can_trade and ctx.breakout_detector:
                ctx.state = SessionState.WATCHING_FOR_BREAKOUT
                logger.info(
                    f"[{session.value}] {self.instrument} -> WATCHING_FOR_BREAKOUT "
                    f"(re-entry after stop, trades: {ctx.trade_count}/{TradingConfig.MAX_TRADES_PER_SESSION})"
                )
            else:
                ctx.state = SessionState.SESSION_CLOSED
                reason_txt = "max trades reached" if not ctx.can_trade else f"{reason} exit"
                logger.info(
