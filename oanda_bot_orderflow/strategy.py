"""MS-ORB strategy logic for range detection and breakout signals."""

import logging
from dataclasses import dataclass, field
from datetime import datetime, time
from enum import Enum
from typing import Optional

from config import SessionConfig, SessionName, TradingConfig
from oanda_client import Candle

logger = logging.getLogger(__name__)


class Direction(Enum):
    """Trade direction."""
    LONG = "LONG"
    SHORT = "SHORT"
    NONE = "NONE"


@dataclass
class OpeningRange:
    """Opening range data for a session."""
    session: SessionName
    instrument: str
    high: float
    low: float
    start_time: datetime
    end_time: datetime
    candles_used: int = 0

    @property
    def range_size(self) -> float:
        """Size of the opening range."""
        return self.high - self.low

    def __str__(self) -> str:
        return (
            f"OpeningRange({self.session.value} {self.instrument}: "
            f"H={self.high:.5f} L={self.low:.5f} Size={self.range_size:.5f})"
        )


@dataclass
class BreakoutSignal:
    """Signal generated when price breaks the opening range."""
    session: SessionName
    instrument: str
    direction: Direction
    entry_price: float
    stop_loss: float
    take_profit: float
    opening_range: OpeningRange
    signal_time: datetime
    breakout_candle: Candle
    risk_per_unit: float
    rr_target: float

    @property
    def risk_reward_ratio(self) -> float:
        """Calculate actual risk-reward ratio."""
        risk = abs(self.entry_price - self.stop_loss)
        reward = abs(self.take_profit - self.entry_price)
        return reward / risk if risk > 0 else 0

    def __str__(self) -> str:
        return (
            f"BreakoutSignal({self.direction.value} {self.instrument} @ {self.entry_price:.5f}, "
            f"SL={self.stop_loss:.5f}, TP={self.take_profit:.5f}, RR={self.risk_reward_ratio:.2f})"
        )


class RangeRecorder:
    """Records and tracks the opening range for a session."""

    def __init__(self, session: SessionName, instrument: str):
        """Initialize range recorder.

        Args:
            session: Session name (TOKYO, LONDON, NY).
            instrument: Instrument name.
        """
        self.session = session
        self.instrument = instrument
        self._high: Optional[float] = None
        self._low: Optional[float] = None
        self._start_time: Optional[datetime] = None
        self._candles: list[Candle] = []
        self._complete = False

    def reset(self) -> None:
        """Reset the range recorder for a new session."""
        self._high = None
        self._low = None
        self._start_time = None
        self._candles = []
        self._complete = False
        logger.debug(f"Range recorder reset for {self.session.value} {self.instrument}")

    def add_candle(self, candle: Candle) -> None:
        """Add a candle to the range calculation.

        Args:
            candle: Complete 5-minute candle from the range period.
        """
        if self._complete:
            return

        if not candle.complete:
            logger.debug(f"Skipping incomplete candle: {candle.time}")
            return

        if self._start_time is None:
            self._start_time = candle.time

        # Update high/low
        if self._high is None or candle.high > self._high:
            self._high = candle.high

        if self._low is None or candle.low < self._low:
            self._low = candle.low

        self._candles.append(candle)
        logger.debug(
            f"Range update {self.session.value} {self.instrument}: "
            f"H={self._high:.5f} L={self._low:.5f} (candles: {len(self._candles)})"
        )

    def finalize(self) -> Optional[OpeningRange]:
        """Finalize the opening range after the range period ends.

        Returns:
            OpeningRange object if valid, None otherwise.
        """
        if self._high is None or self._low is None:
            logger.warning(f"Cannot finalize range: no candles recorded for {self.session.value}")
            return None

        if len(self._candles) < 1:
            logger.warning(f"Cannot finalize range: insufficient candles for {self.session.value}")
            return None

        self._complete = True
        end_time = self._candles[-1].time if self._candles else self._start_time

        opening_range = OpeningRange(
            session=self.session,
            instrument=self.instrument,
            high=self._high,
            low=self._low,
            start_time=self._start_time,
            end_time=end_time,
            candles_used=len(self._candles),
        )

        logger.info(f"Opening range finalized: {opening_range}")
        return opening_range

    @property
    def is_complete(self) -> bool:
        """Check if range recording is complete."""
        return self._complete

    @property
    def current_high(self) -> Optional[float]:
        """Current range high."""
        return self._high

    @property
    def current_low(self) -> Optional[float]:
        """Current range low."""
        return self._low


class BreakoutDetector:
    """Detects breakout signals from completed candles."""

    def __init__(
        self,
        opening_range: OpeningRange,
        rr_target: float = None,
    ):
        """Initialize breakout detector.

        Args:
            opening_range: The opening range to watch for breakouts.
            rr_target: Risk-reward target. Defaults to TradingConfig.RR_TARGET.
        """
        self.opening_range = opening_range
        self.rr_target = rr_target or TradingConfig.RR_TARGET

        self._breakout_up_triggered = False
        self._breakout_down_triggered = False
        self._last_processed_candle: Optional[datetime] = None

    def reset(self) -> None:
        """Reset breakout detector state."""
        self._breakout_up_triggered = False
        self._breakout_down_triggered = False
        self._last_processed_candle = None

    def check_breakout(self, candle: Candle) -> Optional[BreakoutSignal]:
        """Check if a candle represents a breakout.

        A breakout occurs when:
        - LONG: 5-min candle CLOSES above the range high
        - SHORT: 5-min candle CLOSES below the range low

        Args:
            candle: Complete 5-minute candle to check.

        Returns:
            BreakoutSignal if breakout detected, None otherwise.
        """
        if not candle.complete:
            return None

        # Avoid processing the same candle twice
        if self._last_processed_candle and candle.time <= self._last_processed_candle:
            return None

        self._last_processed_candle = candle.time

        # Check for upside breakout (LONG)
        if candle.close > self.opening_range.high:
            if not self._breakout_up_triggered:
                signal = self._create_signal(candle, Direction.LONG)
                if signal:
                    self._breakout_up_triggered = True
                    logger.info(f"Upside breakout detected: {signal}")
                    return signal

        # Check for downside breakout (SHORT)
        if candle.close < self.opening_range.low:
            if not self._breakout_down_triggered:
                signal = self._create_signal(candle, Direction.SHORT)
                if signal:
                    self._breakout_down_triggered = True
                    logger.info(f"Downside breakout detected: {signal}")
                    return signal

        return None

    def _create_signal(
        self,
        candle: Candle,
        direction: Direction,
    ) -> Optional[BreakoutSignal]:
        """Create a breakout signal.

        Args:
            candle: The breakout candle.
            direction: Trade direction.

        Returns:
            BreakoutSignal or None if invalid.
        """
        # Entry at market (candle close)
        entry_price = candle.close

        if direction == Direction.LONG:
            # Long: Stop at breakout candle low
            stop_loss = candle.low
            risk = entry_price - stop_loss

            if risk <= 0:
                logger.warning(f"Invalid long signal: entry={entry_price}, stop={stop_loss}")
                return None

            take_profit = entry_price + (risk * self.rr_target)

        else:  # SHORT
            # Short: Stop at breakout candle high
            stop_loss = candle.high
            risk = stop_loss - entry_price

            if risk <= 0:
                logger.warning(f"Invalid short signal: entry={entry_price}, stop={stop_loss}")
                return None

            take_profit = entry_price - (risk * self.rr_target)

        return BreakoutSignal(
            session=self.opening_range.session,
            instrument=self.opening_range.instrument,
            direction=direction,
            entry_price=entry_price,
            stop_loss=stop_loss,
            take_profit=take_profit,
            opening_range=self.opening_range,
            signal_time=candle.time,
            breakout_candle=candle,
            risk_per_unit=abs(entry_price - stop_loss),
            rr_target=self.rr_target,
        )

    def can_reenter(self, direction: Direction) -> bool:
        """Check if re-entry is allowed in a direction.

        Re-entry is allowed if:
        - Previously stopped out and price re-breaks in same direction
        - Price breaks in the opposite direction (first time)

        Args:
            direction: Direction to check.

        Returns:
            True if re-entry is allowed.
        """
        if direction == Direction.LONG:
            # Allow if: first break up, or opposite side already triggered
            return not self._breakout_up_triggered or self._breakout_down_triggered
        else:
            # Allow if: first break down, or opposite side already triggered
            return not self._breakout_down_triggered or self._breakout_up_triggered

    def reset_direction(self, direction: Direction) -> None:
        """Reset a direction's trigger after a stop out.

        This allows re-entry if price breaks again.

        Args:
            direction: Direction to reset.
        """
        if direction == Direction.LONG:
            self._breakout_up_triggered = False
            logger.debug("Long breakout trigger reset (allow re-entry)")
        else:
            self._breakout_down_triggered = False
            logger.debug("Short breakout trigger reset (allow re-entry)")


def calculate_position_size(
    account_balance: float,
    risk_percent: float,
    entry_price: float,
    stop_price: float,
) -> int:
    """Calculate position size based on risk.

    Args:
        account_balance: Current account balance.
        risk_percent: Risk as decimal (0.01 = 1%).
        entry_price: Entry price.
        stop_price: Stop loss price.

    Returns:
        Number of units to trade.
    """
    risk_amount = account_balance * risk_percent
    risk_per_unit = abs(entry_price - stop_price)

    if risk_per_unit <= 0:
        return 0

    units = int(risk_amount / risk_per_unit)
    return max(units, 0)


# ─────────────────────────────────────────────────────────────
#  Order flow functions
# ─────────────────────────────────────────────────────────────


@dataclass
class OrderFlowResult:
    """Result of order flow filter evaluation."""
    cvd_range: float
    cvd_aligned: bool
    volume_ramp: float
    passes_filter: bool
    rejection_reason: str = ""


def compute_candle_delta(candle: Candle) -> int:
    """Classify candle volume as buy (+) or sell (-) based on close vs open."""
    if candle.close > candle.open:
        return candle.volume
    elif candle.close < candle.open:
        return -candle.volume
    return 0


def compute_cvd(candles: list[Candle]) -> float:
    """Cumulative Volume Delta over a list of candles."""
    return sum(compute_candle_delta(c) for c in candles)


def compute_volume_ramp(candles: list[Candle]) -> float:
    """Volume slope over candles, normalized by average.

    Positive = ramping up, negative = fading.
    """
    if len(candles) < 2:
        return 0.0
    volumes = [c.volume for c in candles]
    avg_vol = sum(volumes) / len(volumes)
    if avg_vol <= 0:
        return 0.0
    return (volumes[-1] - volumes[0]) / avg_vol


def check_order_flow(
    range_candles: list[Candle],
    pre_breakout_candles: list[Candle],
    direction: Direction,
    require_cvd_aligned: bool = True,
    min_volume_ramp: float = 0.0,
) -> OrderFlowResult:
    """Evaluate order flow filter for a breakout signal.

    Args:
        range_candles: Candles from the opening range period.
        pre_breakout_candles: Last 3 candles before the breakout candle.
        direction: Breakout direction (LONG or SHORT).
        require_cvd_aligned: Whether CVD alignment is required.
        min_volume_ramp: Minimum volume ramp threshold.

    Returns:
        OrderFlowResult with filter decision and metadata.
    """
    cvd = compute_cvd(range_candles)
    ramp = compute_volume_ramp(pre_breakout_candles)

    cvd_aligned = (
        (direction == Direction.LONG and cvd > 0)
        or (direction == Direction.SHORT and cvd < 0)
    )

    passes = True
    reasons = []

    if require_cvd_aligned and not cvd_aligned:
        passes = False
        reasons.append(f"CVD not aligned (cvd={cvd:.0f}, dir={direction.value})")

    if ramp < min_volume_ramp:
        passes = False
        reasons.append(f"volume ramp too low ({ramp:.2f} < {min_volume_ramp:.2f})")

    return OrderFlowResult(
        cvd_range=cvd,
        cvd_aligned=cvd_aligned,
        volume_ramp=ramp,
        passes_filter=passes,
        rejection_reason="; ".join(reasons),
    )


if __name__ == "__main__":
    # Test the strategy logic
    logging.basicConfig(level=logging.DEBUG)

    from datetime import datetime, timezone

    # Create a test opening range
    now = datetime.now(timezone.utc)

    recorder = RangeRecorder(SessionName.NY, "XAU_USD")

    # Simulate 6 candles of range period (30 mins = 6 x 5-min candles)
    test_candles = [
        Candle(now, 2050.00, 2055.00, 2048.00, 2052.00, 1000, True),
        Candle(now, 2052.00, 2058.00, 2050.00, 2056.00, 1100, True),
        Candle(now, 2056.00, 2060.00, 2054.00, 2058.00, 1200, True),
        Candle(now, 2058.00, 2062.00, 2055.00, 2060.00, 1300, True),
        Candle(now, 2060.00, 2063.00, 2057.00, 2061.00, 1400, True),
        Candle(now, 2061.00, 2065.00, 2059.00, 2064.00, 1500, True),
    ]

    for candle in test_candles:
        recorder.add_candle(candle)

    opening_range = recorder.finalize()
    print(f"\nOpening Range: {opening_range}")
    print(f"Range High: {opening_range.high}")
    print(f"Range Low: {opening_range.low}")
    print(f"Range Size: {opening_range.range_size}")

    # Test breakout detection
    detector = BreakoutDetector(opening_range, rr_target=5.0)

    # Simulate a breakout candle (closes above range high)
    breakout_candle = Candle(now, 2064.00, 2068.00, 2063.00, 2067.00, 2000, True)

    signal = detector.check_breakout(breakout_candle)
    if signal:
        print(f"\nBreakout Signal: {signal}")
        print(f"Direction: {signal.direction.value}")
        print(f"Entry: {signal.entry_price}")
        print(f"Stop Loss: {signal.stop_loss}")
        print(f"Take Profit: {signal.take_profit}")
        print(f"Risk per unit: {signal.risk_per_unit}")
        print(f"RR Ratio: {signal.risk_reward_ratio}")

    # Test position sizing
    account = 10000.0
    risk_pct = 0.01  # 1%

    if signal:
        units = calculate_position_size(
            account, risk_pct, signal.entry_price, signal.stop_loss
        )
        print(f"\nPosition Size: {units} units")
        print(f"Risk Amount: ${account * risk_pct:.2f}")
        print(f"Risk per Unit: ${signal.risk_per_unit:.5f}")
